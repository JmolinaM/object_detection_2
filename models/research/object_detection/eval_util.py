...
# TODO: Add tests.
def visualize_detection_results(sess,result_dict,
                                tag,
                                global_step,
                                categories,
                                summary_dir='',
  ...
  #m = detection_scores.shape 
  #print(m)
  #print(detection_boxes)
  #print(np.shape(groundtruth_boxes_b))
  matchGT_matrix = list()
  FP_matrix= list()
...
def save_values_matrix(sess, result_dict, max_num_prediction=20):
    if not set([
        'original_image', 'detection_boxes', 'detection_scores',
        'detection_classes'
    ]).issubset(set(result_dict.keys())):
      ...
      for j in range(len_compare[0]):
        ...
        if j==99 and max_score<0.6:
          if flag==0:
            ...
            matchGT_matrix.append([0, groundtruth_classes[i], detection_scores[j]])
        if IrU<0.2 and i==1 and np.max(detection_scores[j])>0.5:
          FP_matrix.append([detection_classes[j], 0, detection_scores[j]])
